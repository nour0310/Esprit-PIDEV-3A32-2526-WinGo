...
public class SignupController implements Initializable {
    ...
    private static final Pattern NAME_PATTERN = Pattern.compile("^[a-zA-Z\\u00C0-\\u00FF\\s'-]{2,50}$");
    ...
    public void register() {
        ...
        if (!NAME_PATTERN.matcher(nom).matches()) {
            showError("âš  Nom: letters only, 2-50 characters");
            return;
        }

        // Validate prenom
        if (!NAME_PATTERN.matcher(prenom).matches()) {
            showError("âš  Prenom: letters only, 2-50 characters");
            return;
        }

        // Validate telephone (8-15 digits)
        if (!PHONE_PATTERN.matcher(telephone).matches()) {
            showError("âš  Telephone: 8-15 digits only");
            return;
        }
        ...
        try {
            ...
            if (age < 1 || age > 120) {
                showError("âš  Age must be between 1 and 120");
                return;
            }
        } catch (NumberFormatException e) {
            showError("âš  Age must be a valid number");
            return;
        }

        // Validate email format
        if (!EMAIL_PATTERN.matcher(email).matches()) {
            showError("âš  Invalid email format");
            return;
        }

        // Validate password (min 6 chars)
        if (password.length() < 6) {
            showError("âš  Password must be at least 6 characters");
            return;
        }
        ...
        try {
            ...
            for (Utilisateur u : service.afficher()) {
                if (u.getEmail().equalsIgnoreCase(email)) {
                    showError("âš  Email already registered");
                    return;
                }
            }
            ...
            messageLabel.setText("âœ… Account created! Please verify your email.");
            ...
            if (e.getMessage() != null && e.getMessage().contains("Data truncated")) {
                showError("âŒ Database 'type' column too small. Run: ALTER TABLE utilisateur MODIFY type VARCHAR(50);");
            } else {
                showError("âŒ Error creating account: " + e.getMessage());
            }
            e.printStackTrace();
        } catch (Exception e) {
            showError("âŒ Error creating account");
            e.printStackTrace();
        }
    }
    ...
    private void goToVerification(String email, String code) {
        try {
            ...
            e.printStackTrace();
            showError("âŒ Error navigating to verification: " + e.getMessage());
        }
    }
