...
public class BlogController implements Initializable {
    ...
    @FXML private HBox detailTagsContainer;
    ...
    private void updateProfileInfo() {
        if (Session.getCurrentUser() != null) {
            ...
            if (sidebarUserName != null) {
                sidebarUserName.setText(u.getNom() + " " + u.getPrenom());
            }
            if (userAvatarView != null) {
                try {
                    ProfilCRUD pCRUD = new ProfilCRUD();
                    Entites.Profil p = pCRUD.getProfilByUtilisateurId(u.getId());
                    ...
                    System.out.println("⚠️ Sidebar avatar error: " + e.getMessage());
                }
            }
        }
    }
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        loadNotifications();
        updateProfileInfo(); // Called the new method
        ...
        if (navbarController != null) {
            navbarController.getSearchBar().setVisible(true);
            navbarController.getSearchBar().setManaged(true);
            navbarController.getNavbarSearchField().setPromptText("Rechercher un article...");
            navbarController.getNavbarSearchField().setOnAction(e -> filterArticles());
            navbarController.getNavbarSearchBtn().setOnAction(e -> filterArticles());
        }
    }
    ...
    private void loadNotifications() {
        ...
        try {
            ...
            if (navbarController != null && navbarController.getNotificationBadge() != null) {
                if (nonLues > 0) {
                    navbarController.getNotificationBadge().setText(String.valueOf(nonLues));
                    navbarController.getNotificationBadge().setVisible(true);
                } else {
                    navbarController.getNotificationBadge().setVisible(false);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private void attachListeners() {
        // searchBtn and searchField are now handled via navbarController in initialize
        ...
        if (navbarController != null) {
            navbarController.getNotificationBtn().setOnAction(e -> afficherNotifications());
        }
    ...
    private void afficherNotifications() {
        try {
            ...
            Button notificationButton = navbarController.getNotificationBtn();
    ...
    private boolean validateTitre() {
        ...
        if (titre == null || titre.trim().isEmpty()) {
            ...
            showError(titreError, "Le titre doit faire entre 3 et 100 caractères.");
            return false;
        } else {
    ...
    private void showDetailView(Blog blog) {
        ...
        // --- GESTION DES TAGS (FIXÉ ET CRÉATIF) ---
        detailTagsContainer.getChildren().clear();
        if (blog.getTags() != null && !blog.getTags().isEmpty()) {
            for (Tag tag : blog.getTags()) {
                ...
                detailTagsContainer.getChildren().add(tagLabel);
            }
        }
        ...
        detailViewScroll.setVvalue(0.0); // Retour en haut de page
        articleFormScroll.setVisible(false);
        articleFormScroll.setManaged(false);
    }
    ...
    private void filterArticles() {
        String search = "";
        if (navbarController != null && navbarController.getNavbarSearchField() != null) {
            search = navbarController.getNavbarSearchField().getText().toLowerCase();
        }
        ...
        List<Blog> filtered = blogList.stream()
                .filter(b -> (searchFinal.isEmpty() ||
                        (b.getTitre() != null && b.getTitre().toLowerCase().contains(searchFinal)) ||
                        (b.getContenu() != null && b.getContenu().toLowerCase().contains(searchFinal)) ||
                        (b.getAuteurNom() != null && b.getAuteurNom().toLowerCase().contains(searchFinal))))
    ...
    @FXML
    public void goSettings(javafx.event.ActionEvent event) {
        // Rediriger vers Home.fxml où le profil est géré.
        loadPage("/Home.fxml");
    }

    @FXML
    public void goBlog(javafx.event.ActionEvent event) {
        try {
            loadBlogs();
            showListView();
            if (statusLabel != null) statusLabel.setText("✅ Blog actualisé.");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    @FXML
    public void goSupport(javafx.event.ActionEvent event) {
        loadPage("/Support.fxml");
    }

    @FXML
    public void toggleDarkMode(javafx.event.ActionEvent event) {
        if (rootPane != null && darkModeBtn != null && backgroundPane != null) {
    ...
    private void loadPage(String path) {
        try {
            ...
            e.printStackTrace();
            showError("Erreur de navigation", "Impossible de charger " + path);
        }
    }
}
