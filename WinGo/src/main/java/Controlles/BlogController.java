...
public class BlogController implements Initializable {
    ...
    private final TagCRUD tagCRUD = new TagCRUD(); // Nouveau service pour les tags
    ...
    @FXML private TextField tagsField; // Nouveau champ pour les tags
    ...
    private void loadInitialData() {
        try {
            ...
            loadLikes();
            loadRatings();
            loadBlogs(); // Cette méthode charge maintenant aussi les tags
            updateStats();
    ...
    private void loadBlogs() throws SQLException {
        blogList.clear();
        List<Blog> blogs = blogCRUD.afficher(); // Déjà avec les tags (si BlogCRUD modifié)
        // Si BlogCRUD ne charge pas les tags, on peut les charger ici manuellement
        for (Blog b : blogs) {
    ...
    private VBox createBlogCard(Blog blog) {
        ...
        content.getChildren().addAll(auteur, date, categorie, extraitLabel, commentCount, likeBox, ratingBox);
    ...
    private void showDetailView(Blog blog) {
        ...
        // On récupère l'index de detailCommentairesPane et on insère avant
    ...
    // ========== GESTION DES TAGS ==========
    /**
     * Traite la chaîne de tags saisie : crée les tags si nécessaire et les associe à l'article.
     * @param articleId l'ID de l'article
     * @param tagsInput la chaîne de tags (ex: "voyage, été, plage")
     * @throws SQLException
     */
    ...
    private void supprimerBlog(Blog blog) {
        ...
        confirm.showAndWait().ifPresent(r -> {
            if (r == ButtonType.YES) {
                try {
                    // Les tags sont automatiquement supprimés par CASCADE
    ...
    private void ajouterBlog() {
        ...
        try {
            ...
            blogCRUD.ajouter(b); // Après cette ligne, b.getId() est disponible
    ...
    private void refreshData() throws SQLException {
        loadBlogs(); // recharge les articles avec leurs tags
        loadAllComments();
        loadLikes();
        loadRatings();
        filterArticles();
