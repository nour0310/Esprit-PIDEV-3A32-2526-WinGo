...
public class LoginController implements Initializable {
    ...
    public void login() {
        ...
        if (email.isEmpty() || password.isEmpty()) {
            errorLabel.setText("âš  Please fill all fields");
            return;
        }

        if (!EMAIL_PATTERN.matcher(email).matches()) {
            errorLabel.setText("âš  Invalid email format");
            return;
        }

        if (password.length() < 6) {
            errorLabel.setText("âš  Password must be at least 6 characters");
            return;
        }
        ...
        if (enteredCaptcha.isEmpty()) {
            errorLabel.setText("âš  Veuillez saisir le code CAPTCHA");
            return;
        }
        if (!enteredCaptcha.equalsIgnoreCase(currentCaptcha)) {
            errorLabel.setText("âŒ Code CAPTCHA incorrect");
            refreshCaptcha();
            return;
        }
        ...
        try {
            for (Utilisateur u : service.afficher()) {
                ...
                if (u.getEmail().equals(email)
                    ...
                    // admin â†’ AdminDashboard; user â†’ WinGo Shop
                    // Skip verification for admins
                    if (!"admin".equalsIgnoreCase(u.getType()) && !u.isVerified()) {
                        errorLabel.setText("âš  Veuillez vÃ©rifier votre email.");
            ...
            // âŒ If no match found
            errorLabel.setText("âŒ Email or password incorrect");

        } catch (Exception e) {
            errorLabel.setText("âš  Error connecting to database");
            e.printStackTrace();
        }
    }
    ...
    private void loadPage(String path) {
        try {
            ...
            errorLabel.setText("âš  Error loading page");
            e.printStackTrace();
        }
    }
}
