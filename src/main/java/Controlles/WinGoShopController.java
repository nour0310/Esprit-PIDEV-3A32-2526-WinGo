...
public class WinGoShopController {
    ...
    // ==================== BECOME COMMERCANT (champs FXML cachés, conservés pour compatibilité) ====================
    ...
    private void setupRealtimeValidation() {
        styleErrorLabel(errNom);  styleErrorLabel(errCat);   styleErrorLabel(errPrix);
        styleErrorLabel(errStock);styleErrorLabel(errImage); styleErrorLabel(errRegion);
        styleErrorLabel(errDesc); styleErrorLabel(errEmail); styleErrorLabel(errPassword);
        styleErrorLabel(errBcNom);styleErrorLabel(errBcPhone);styleErrorLabel(errBcType);
        ...
        if (imageField != null) {
            ...
            imageField.textProperty().addListener((obs, old, val) -> {
                ...
                if (val.contains("encrypted-tbn") || val.contains("gstatic.com/images"))
                    showInlineError(imageField, errImage, "🚫 URL Google Images non supportée → Unsplash ou Wikipedia");
    ...
    private boolean hasError(Label errLabel) { return errLabel.isVisible() && !errLabel.getText().isBlank(); }
            ...
            categorieCombo.setButtonCell(new ListCell<>() {
                @Override protected void updateItem(String item, boolean empty) {
                    ...
                    if (empty || item == null) {
                        ...
                        setStyle("-fx-background-color: transparent; -fx-text-fill: rgba(255,255,255,0.55); -fx-font-size: 12px;");
                    } else {
                        setText(item);
                        setStyle("-fx-background-color: transparent; -fx-text-fill: white; -fx-font-weight: 700; -fx-font-size: 12px;");
                    }
                }
            });
        }
    ...
    @FXML private void onCategorieChanged() {
        ...
        if (isAutre) {
            ...
            errCat.setVisible(false); errCat.setManaged(false);
    ...
    @FXML public void doLogin() {
        ...
        else if (!email.matches("^[\\w._%+\\-]+@[\\w.\\-]+\\.[a-zA-Z]{2,}$")) { showInlineError(emailField, errEmail, "⚡ Format invalide"); valid = false; }
        ...
        if (!valid) { loginStatusLabel.setStyle("-fx-text-fill: #FF0054; -fx-font-weight: 800;"); loginStatusLabel.setText("⚠ Corrige les erreurs."); return; }
    ...
    private String getNomPrenomUtilisateur(int idUser) {
        ...
        try (PreparedStatement pst = MyBD.getInstance().getConn().prepareStatement(sql)) {
            pst.setInt(1, idUser);
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                String n = rs.getString("nom"); String p = rs.getString("prenom");
                String full = (p != null ? p : "") + " " + (n != null ? n : "");
    ...
    private HBox createProductCard(Produit p) {
        ...
        clip.setArcWidth(40); clip.setArcHeight(40); imageBlock.setClip(clip);
        ImageView iv = new ImageView();
        iv.setFitWidth(230); iv.setFitHeight(185); iv.setPreserveRatio(false); iv.setSmooth(true);
        ...
        nom.setStyle("-fx-text-fill: #87CEEB; -fx-font-weight: 900; -fx-font-size: 19px;");
        ...
        catBadge.setStyle("-fx-background-color: #EEF2FF; -fx-text-fill: #6366F1; -fx-font-weight: 800; -fx-font-size: 10px; -fx-background-radius: 6; -fx-padding: 3 8;");

        HBox priceBadge = new HBox(6); priceBadge.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        priceBadge.setMaxWidth(170); priceBadge.setStyle("-fx-background-color: #FFF7ED; -fx-background-radius: 10; -fx-padding: 5 14;");
        Label ticketIco = new Label("🎫");
        Label prix = new Label(String.format("%.2f TND", p.getPrix()));
        prix.setStyle("-fx-text-fill: #C2410C; -fx-font-weight: 900; -fx-font-size: 17px;");
        ...
        descLbl.setStyle("-fx-text-fill: #475569; -fx-font-size: 12px;"); descLbl.setMaxWidth(500); descLbl.setWrapText(true);
        ...
        statusLbl.setStyle("-fx-text-fill: " + (p.getStock() > 0 ? "#10B981" : "#EF4444") + "; -fx-font-weight: 700; -fx-font-size: 11px;");
    ...
    private void addProductToCart(Produit p) {
        ...
        try {
            ...
            refreshCartUI(); showAlert("✅ Ajouté!", p.getNom() + " ajouté au panier.");
    ...
    @FXML public void showPanierDialog() {
        ...
        Label panelTitle = new Label("🛒  Mon Panier");
        panelTitle.setStyle("-fx-font-weight: 900; -fx-font-size: 16px; -fx-text-fill: #1E293B;");
        Region hSpacer = new Region(); HBox.setHgrow(hSpacer, Priority.ALWAYS);
        Button closeBtn = new Button("✕");
        closeBtn.setStyle("-fx-background-color: #F1F5F9; -fx-text-fill: #64748B; -fx-background-radius: 999; -fx-font-weight: 900; -fx-font-size: 12px; -fx-min-width: 30; -fx-min-height: 30; -fx-cursor: hand; -fx-padding: 0;");
        ...
        colHeader.setStyle("-fx-padding: 10 20; -fx-background-color: #F8F9FA; -fx-border-color: #E2E8F0; -fx-border-width: 0 0 1 0;");
        ...
        Label sousTotalLbl = new Label("0.000 TND");
        sousTotalLbl.setStyle("-fx-font-weight: 900; -fx-font-size: 14px; -fx-text-fill: #1E293B;");
        Label totalLbl = new Label("0.000 TND");
        totalLbl.setStyle("-fx-font-weight: 900; -fx-font-size: 15px; -fx-text-fill: #1E293B;");

        VBox itemsBox = new VBox(0);
        if (items.isEmpty()) {
            Label empty = new Label("🛒  Votre panier est vide");
            empty.setStyle("-fx-text-fill: #94A3B8; -fx-font-size: 15px; -fx-font-weight: 700; -fx-padding: 40 0;");
            itemsBox.setAlignment(javafx.geometry.Pos.CENTER); itemsBox.getChildren().add(empty);
        } else {
            for (CartItem it : items) itemsBox.getChildren().add(buildPanierRow(it, itemsBox, sousTotalLbl, totalLbl, overlay));
        }
        ...
        Button continueBtn = new Button("← POURSUIVRE LES ACHATS");
        continueBtn.setStyle("-fx-background-color: transparent; -fx-text-fill: #475569; -fx-font-weight: 700; -fx-font-size: 12px; -fx-border-color: #CBD5E1; -fx-border-radius: 6; -fx-background-radius: 6; -fx-padding: 8 16; -fx-cursor: hand;");
        ...
        VBox totalSection = new VBox(0);
        totalSection.setStyle("-fx-background-color: #F8F9FA; -fx-border-color: #E2E8F0; -fx-border-width: 1 0 0 0; -fx-padding: 16 24; -fx-background-radius: 0 0 16 16;");
        Label totalTitle = new Label("TOTAL PANIER");
        totalTitle.setStyle("-fx-font-weight: 900; -fx-font-size: 13px; -fx-text-fill: #1E293B; -fx-padding: 0 0 10 0;");
        HBox stRow = makePanierTotalRow("Sous-total", sousTotalLbl);
        ...
        Label livrInfo = new Label("Livraison à domicile sur toute la Tunisie (48h ouvrable): 4.500 TND");
        livrInfo.setStyle("-fx-font-size: 11px; -fx-text-fill: #64748B;"); livrInfo.setWrapText(true);
        livrRow.getChildren().addAll(livrLbl, livrInfo);
        HBox totRow = makePanierTotalRow("Total", totalLbl);
        Button checkoutBtn = new Button("VALIDER LA COMMANDE");
        ...
        totalSection.getChildren().addAll(totalTitle, stRow, livrRow, totRow, checkoutBtn);
    ...
    private HBox buildPanierRow(CartItem it, VBox itemsBox, Label sousTotalLbl, Label totalLbl, StackPane overlay) {
        ...
        row.setStyle("-fx-padding: 12 20; -fx-border-color: #F1F5F9; -fx-border-width: 0 0 1 0;");

        Button delBtn = new Button("✕");
        delBtn.setStyle("-fx-background-color: #F1F5F9; -fx-text-fill: #94A3B8; -fx-background-radius: 999; -fx-font-size: 10px; -fx-min-width: 22; -fx-max-width: 22; -fx-min-height: 22; -fx-max-height: 22; -fx-cursor: hand; -fx-padding: 0;");
    ...
    private void recalcPanierTotals(Label sousTotalLbl, Label totalLbl) {
        try {
            List<CartItem> current = panierCRUD.getActiveCart(Session.getUserId());
            double sub = current.stream().mapToDouble(CartItem::getSubtotal).sum();
            sousTotalLbl.setText(String.format("%.3f TND", sub));
            totalLbl.setText(String.format("%.3f TND", sub + 4.5));
    ...
    // ==================== BECOME COMMERCANT — OVERLAY AVEC APPROBATION ADMIN ====================
    ...
    public void showBecomeCommercant() {
        ...
        String statut = null;
        try { statut = demandeCRUD.getStatutDemande(Session.getUserId()); }
        catch (Exception e) { e.printStackTrace(); }
        ...
        VBox panel = buildBecomeCommercantPanel(overlay, statutFinal);
        panel.setOnMouseClicked(javafx.event.Event::consume);
        overlay.getChildren().add(panel);
        overlayContainer.getChildren().add(overlay);
    }

    private VBox buildBecomeCommercantPanel(StackPane overlay, String statut) {
        VBox panel = new VBox(0);
        panel.setPrefWidth(560); panel.setMaxWidth(560);
        panel.setStyle(
                "-fx-background-color: #0F172A;" +
                        "-fx-background-radius: 20;" +
                        "-fx-effect: dropshadow(gaussian, rgba(99,102,241,0.45), 60, 0, 0, 0);"
        );
        ...
        Label title = new Label("🏪  Devenir Commerçant");
        title.setStyle("-fx-text-fill: white; -fx-font-weight: 900; -fx-font-size: 17px;");
        Region hsp = new Region(); HBox.setHgrow(hsp, Priority.ALWAYS);
        Button closeBtn = new Button("✕");
        closeBtn.setStyle("-fx-background-color: rgba(255,255,255,0.18); -fx-text-fill: white; -fx-background-radius: 999; -fx-font-weight: 900; -fx-font-size: 12px; -fx-min-width: 32; -fx-min-height: 32; -fx-cursor: hand; -fx-padding: 0;");
        closeBtn.setOnAction(e -> overlayContainer.getChildren().remove(overlay));
        header.getChildren().addAll(title, hsp, closeBtn);

        VBox body = new VBox(14);
        body.setStyle("-fx-padding: 28 28 24 28;");
        ...
        if ("EN_ATTENTE".equals(statut)) {
            ...
            Label sub = new Label("Votre demande a bien été reçue.\nUn administrateur la traitera sous 24-48h ouvrables.");
            sub.setStyle("-fx-text-fill: #94A3B8; -fx-font-size: 13px; -fx-text-alignment: center;");
            sub.setWrapText(true); sub.setMaxWidth(440); sub.setAlignment(javafx.geometry.Pos.CENTER);
            ...
            Label dot = new Label("●"); dot.setStyle("-fx-text-fill: #FFBD00; -fx-font-size: 10px;");
            Label badgeTxt = new Label("EN ATTENTE DE VALIDATION");
            badgeTxt.setStyle("-fx-text-fill: #FFBD00; -fx-font-weight: 900; -fx-font-size: 12px;");
            badge.getChildren().addAll(dot, badgeTxt);
            body.getChildren().addAll(ico, msg, sub, badge);

        } else if ("REFUSE".equals(statut)) {
            // ── STATUT REFUSÉ → re-soumettre ──────────────────
            Label ico = new Label("❌"); ico.setStyle("-fx-font-size: 36px;");
            Label msg = new Label("Demande précédemment refusée");
            msg.setStyle("-fx-text-fill: #F87171; -fx-font-weight: 900; -fx-font-size: 15px;");
            Label sub = new Label("Vous pouvez soumettre une nouvelle demande en complétant le formulaire ci-dessous.");
            ...
            body.getChildren().addAll(ico, msg, sub, sep);
            body.getChildren().addAll(buildFormNodes(body, overlay));

        } else {
            // ── AUCUNE DEMANDE → formulaire vierge ────────────
    ...
    /** Construit les champs + bouton du formulaire "Devenir Commerçant" */
    private List<Node> buildFormNodes(VBox body, StackPane overlay) {
        TextField fNom   = bcField("👤  Nom complet (lettres uniquement)");
        ...
        fMotiv.setPromptText("✍  Décrivez votre activité et vos motivations (min. 20 car.)...");
        fMotiv.setPrefRowCount(4); fMotiv.setWrapText(true);
        fMotiv.setStyle(
                "-fx-background-color: rgba(255,255,255,0.06); -fx-background-radius: 12;" +
                        "-fx-border-color: rgba(255,255,255,0.15); -fx-border-radius: 12;" +
                        "-fx-text-fill: white; -fx-padding: 10 12; -fx-font-size: 13px; -fx-prompt-text-fill: rgba(255,255,255,0.35);"
        );

        Label statusLbl = new Label("");
        statusLbl.setStyle("-fx-text-fill: #F87171; -fx-font-weight: 800; -fx-font-size: 12px;");
        statusLbl.setWrapText(true);

        Button submitBtn = new Button("📨  Soumettre ma demande");
        ...
        submitBtn.setOnAction(e -> handleSubmitDemande(fNom, fPhone, fType, fMotiv, statusLbl, overlay));
    ...
    private void handleSubmitDemande(TextField fNom, TextField fPhone, TextField fType,
                                     TextArea fMotiv, Label statusLbl, StackPane overlay) {
        String nom   = fNom.getText()   == null ? "" : fNom.getText().trim();
        String phone = fPhone.getText() == null ? "" : fPhone.getText().trim();
        String type  = fType.getText()  == null ? "" : fType.getText().trim();
        String motiv = fMotiv.getText() == null ? "" : fMotiv.getText().trim();

        // Validation
        if (nom.isEmpty() || !nom.matches("^[a-zA-ZÀ-ÿ\\s\\-']+$") || nom.length() < 3) {
            statusLbl.setText("⚡ Nom invalide (lettres uniquement, min. 3 caractères)."); bcMarkError(fNom); return;
        }
        if (!phone.replaceAll("[\\s\\-+]","").matches("^(\\+?216)?[2-9]\\d{7}$")) {
            statusLbl.setText("⚡ Téléphone invalide (ex: +216 22 123 456)."); bcMarkError(fPhone); return;
        }
        if (type.isEmpty() || type.length() < 3) {
            statusLbl.setText("⚡ Type de produits obligatoire (min. 3 caractères)."); bcMarkError(fType); return;
        }
        if (motiv.isEmpty() || motiv.length() < 20) {
            statusLbl.setText("⚡ Motivation trop courte (" + motiv.length() + "/20 caractères min.).");
            fMotiv.setStyle(fMotiv.getStyle() + " -fx-border-color: #F87171 !important;"); return;
        }

        // Soumission
        try {
            demandeCRUD.soumettre(Session.getUserId(), nom, phone, type, motiv);
            // Rafraîchir l'overlay avec le statut EN_ATTENTE
            overlayContainer.getChildren().remove(overlay);
            showBecomeCommercant(); // se rouvrira en mode EN_ATTENTE
    ...
    // ─── Helpers UI formulaire BecomeCommercant ───────────────────
    ...
    private void bcMarkError(TextField f) {
        f.setStyle("-fx-background-color: rgba(248,113,113,0.1); -fx-background-radius: 12; -fx-border-color: #F87171; -fx-border-width: 1.5; -fx-border-radius: 12; -fx-text-fill: white; -fx-padding: 12 14; -fx-font-size: 13px;");
    }

    private VBox bcFieldRow(String labelTxt, Control field) {
        VBox box = new VBox(5);
    ...
    private Label bcFieldLabel(String text) {
        Label lbl = new Label(text);
        lbl.setStyle("-fx-text-fill: rgba(255,255,255,0.55); -fx-font-size: 11px; -fx-font-weight: 700;");
        VBox.setMargin(lbl, new javafx.geometry.Insets(4, 0, 0, 0));
        return lbl;
    }

    // ==================== BECOME COMMERCANT : ancienne méthode conservée pour compatibilité FXML ====================
    @FXML
    public void submitBecomeCommercant() {
        // Redirige vers le nouvel overlay
        showBecomeCommercant();
    }

    // ==================== EDIT / DELETE / SAVE PRODUCT ====================
    @FXML public void editSelectedProduct() {
        ...
        nomTextField.setText(selected.getNom()); prixTextField.setText(String.valueOf(selected.getPrix()));
        stockField.setText(String.valueOf(selected.getStock())); categorieField.setText(selected.getCategorie());
        regionField.setText(selected.getRegion()); imageField.setText(selected.getImage());
    ...
    @FXML public void saveProduct() {
        ...
        if ("✏ Autre...".equals(selectedCat)) {
            ...
            if (cat.isEmpty()) { showInlineError(categorieAutreField, errCat, "⚡ Précisez votre catégorie"); return; }
        } else {
            cat = selectedCat == null ? "" : selectedCat.trim();
            if (cat.isEmpty()) { errCat.setText("⚡ Choisissez une catégorie"); errCat.setVisible(true); errCat.setManaged(true); return; }
        }
        ...
        try {
            ...
            if (idTxt.isEmpty()) {
                ...
                produitCRUD.ajouter(p); showAlert("✅ Ajout", "Produit ajouté avec succès.");
            } else {
                int id = Integer.parseInt(idTxt);
                Produit p = new Produit(id, Session.getUserId(), nom, desc, prix, region, cat, stock, img);
                produitCRUD.modifier(p); showAlert("✅ Modifié", "Produit mis à jour avec succès.");
            }
            refreshProducts(); refreshCommercantProducts(); refreshClientProducts();
            clearForm(); showProducts();
    ...
    @FXML public void clearForm() {
        idProduitHidden.clear(); nomTextField.clear(); descriptionField.clear();
        prixTextField.clear(); stockField.clear(); regionField.clear(); imageField.clear();
        ...
        for (TextField f : new TextField[]{nomTextField, descriptionField, prixTextField, stockField, regionField, imageField})
