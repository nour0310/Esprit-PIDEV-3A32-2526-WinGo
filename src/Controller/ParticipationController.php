...
class ParticipationController extends AbstractController
{
    ...
    public function frontNew(int $id_event, Request $request): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $participation->setIdUser($userId);
    ...
    public function clientEdit(int $id_participation, Request $request): Response
    {
        ...
        if (!$participation instanceof Participation || $participation->getIdUser() !== $userId) {
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($newPlaces <= 0) {
                ...
                return $this->redirectToRoute('client_edit_reservation', [
    ...
    public function submitFeedback(Event $event, Request $request): Response
    {
        ...
        if ($event->hasUserFeedback($userId)) {
            if ($request->isXmlHttpRequest()) {
        ...
        if (!in_array($feeling, $valid, true)) {
            if ($request->isXmlHttpRequest()) {
    ...
    public function show(int $id_event, int $id_participation): Response
    {
        ...
        if (!$event instanceof Event || !$participation instanceof Participation) {
