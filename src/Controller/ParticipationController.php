...
class ParticipationController extends AbstractController
{
    ...
    /**
     * Génère l'URL complète du QR code pointant vers la page de réservation.
     */
   private function getQrCodeContent(Participation $participation, Event $event): string
{
    ...
    // Supprimer les caractères non-ASCII pour éviter les erreurs d'encodage
    ...
    public function frontNew(int $id_event, Request $request): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            // Add user to the event group chat (non bloquant)
            try {
                $this->streamChatService->addUserToEventChannel($event, $user);
            } catch (\Exception $e) {
                $this->addFlash('info', 'Le chat de l’événement est momentanément indisponible. Vous pourrez y accéder plus tard.');
            }

            // Générer le QR code avec l'URL de la réservation
    ...
    // ---------- PUBLIC : afficher une réservation par token ----------
    ...
    // ---------- QR code image generation (SVG, sans GD) ----------
    ...
    public function qrCode(int $id_participation): Response
    {
        ...
        $qrContent = $this->getQrCodeContent($participation, $event);
        $svgData = $this->qrCodeService->generateQrCode($qrContent);
        return new Response($svgData, 200, ['Content-Type' => 'image/svg+xml']);
    }
}