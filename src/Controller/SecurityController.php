...
class SecurityController extends AbstractController
{
    private function buildMailerErrorMessage(\Throwable $exception): string
    {
        ...
        if (str_contains($message, '535') || str_contains($message, 'Authentication unsuccessful')) {
            return "Le compte a été créé, mais l'envoi d'email a échoué. Le service SMTP actuel refuse l'authentification. Vérifiez MAILER_DSN ou remplacez-le par un autre provider mail comme Brevo, Gmail ou Mailtrap.";
        }
        return sprintf("Le compte a été créé, mais l'envoi d'email a échoué : %s", $message);
    }
    ...
    {
        // Récupération de l'erreur d'authentification (si identifiants incorrects)
        ...
        // La vérification du captcha est désormais gérée par CaptchaValidationSubscriber
        // (écouteur d'événement CheckPassportEvent) pour garantir qu'elle ait lieu
        // avant toute authentification réussie.
    ...
    public function register(
        ...
        if ($request->isMethod('POST')) {
            ...
            if (!$userCaptcha || strtolower($userCaptcha) !== strtolower($sessionCaptcha)) {
                ...
                // Captcha valide : on le supprime pour éviter les réutilisations
                $request->getSession()->remove('captcha_code');
                ...
                $user->setGenre(trim($request->request->get('genre', '')));
                $user->setType('CLIENT');
